...
public class Game : MonoBehaviour {
    ...
    //MONTE CARLO
    data.army_data CloneArmy(data.army_data original) {
    ...
    int SimulateMoveDataOnly(data.AIMove move, int color) {
        data.army_data army = data.mem.get_army(color);
        ref data.chess_piece attacker = ref army.troop_list[move.piece_index];
        ...
        if (move.isAttack) {
            ...
            target.rect = null; 
            ClearCell(move.targetX, move.targetY);

            if (target.piece_type == 5) return 1; //note that this team win
        }

        ClearCell(attacker.x, attacker.y);
        attacker.x = move.targetX;
        attacker.y = move.targetY;
        SetCell(move.targetX, move.targetY, color, move.piece_index);
        
        return 0;//no one win
    }

    data.AIMove CalculateMCTSMove(int ai_color) {
