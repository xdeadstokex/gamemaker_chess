...
public static class move_plate_util {
	...
	public static void spawn_plate(ref data.chess_piece cp, int idx, int color) {
        ...
        switch (cp.piece_type) {
            case 0: move_plate_util.spawn_pawn_plate(ref cp, idx, color, pawnDir);  break;
    ...
    public static void spawn_pawn_plate(ref data.chess_piece cp, int i, int c, int dir) {
        int startRow = (cp.player_color == 0) ? 1 : 6;
        int steps    = (cp.y == startRow) ? 2 : 1;
        move_plate_util.ray_plate(ref cp,i,c,  0, dir, steps, 1, skip_obs:false, capture:false);
        move_plate_util.ray_plate(ref cp,i,c,  1, dir, 1,     1, skip_obs:false, capture_only:true);
        move_plate_util.ray_plate(ref cp,i,c, -1, dir, 1,     1, skip_obs:false, capture_only:true);
    }
    ...
    public static void spawn_plate(int piece_index, int piece_color, int mx, int my, bool isAttack) {
        ...
        data.move_plate mp;
        mp.rect                = rect_2d.create(board_util.board_to_world(mx), board_util.board_to_world(my), -2f); // z=-2, above tile z=0 below piece z=-1
        mp.attack              = isAttack;
