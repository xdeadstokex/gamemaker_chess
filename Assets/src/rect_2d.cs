...
public class rect_2d : MonoBehaviour {
    ...
    public void set_sprite_scale(float x_scale, float y_scale){ visual.transform.localScale = new Vector3(x_scale, y_scale, 1f); }
    public void reset_visual_scale(){ visual.transform.localScale = Vector3.one; }
	public void set_collider_size(float x_scale, float y_scale){ col.size = new Vector2(x_scale, y_scale); }
    public void fit_collider_to_sprite(Sprite s){ if (s != null){ col.size = s.bounds.size; } }
    public float board_to_world(int v){ return v * 1.28f - 4.48f; }
